		...
		if !isValidRes {
			...
			for fieldName := range res.Fields {
				...
				for i := range field.Symbols {
					errors = append(errors, validateEnrichSymbol(&field.Symbols[i], false)...) // FIXME
				}
				if field.Symbol.OID != "" {
...
func validateEnrichSymbol(symbol *SymbolConfig, canSendAsConstant bool) []string {
	...
	if symbol.OID == "" {
		if canSendAsConstant && !symbol.SendAsConstant {
			errors = append(errors, fmt.Sprintf("symbol oid or send_as_one missing: name=`%s` oid=`%s`", symbol.Name, symbol.OID)) // FIXME
	...
	if symbol.SendAsConstant && !canSendAsConstant {
		errors = append(errors, fmt.Sprintf("symbol can only be sent as one in table"))
	}
	return errors
}
