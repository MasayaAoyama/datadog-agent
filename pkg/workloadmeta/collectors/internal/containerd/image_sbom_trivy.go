...
func (c *collector) startSBOMCollection(ctx context.Context) error {
	...
	c.trivyScanner, err = trivy.GetScanner(config.Datadog)
	if err != nil {
...
func (c *collector) extractBOMWithTrivy(ctx context.Context, storedImage *workloadmeta.ContainerImageMetadata) error {
	...
	return scanFunc(storedImage, containerdImage, c.containerdClient, func(report sbom.Report, at time.Time, duration time.Duration) {
